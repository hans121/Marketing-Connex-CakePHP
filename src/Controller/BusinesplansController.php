...
/**
 * Businesplans Controller
 *
 * @property App\Model\Table\BusinesplansTable $Businesplans
 */
class BusinesplansController extends AppController {
 ...
 */
	public function add($dupid=0,$cmpid=0,$fqid=0) {
		...
		$partners = $this->Businesplans->Partners->find('list');
                /*
                 * Section to restrict partner to submit single business plan for a quarter
                 */
	...
	/*
	 * Function to update campaigns of a business plan....
	 */
 ...
 */
	public function delete($id = null) {
        ...
        public function getcampaignsofquarter(){
           $this->layout = 'ajax';
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                $campaigns  =   $this->Campaigns->find('list')
                                                ->where(['financialquarter_id'=>$this->request->data['qtid'],'vendor_id'=>$this->Auth->user('vendor_id'),'id NOT IN'=>$pcmp_ids])
                                                ->all();
                $this->set('campaigns', $campaigns);
                $bpcampaigns    =   array();
        ...
        /*
         * Function to find the financial quarter is added to any business plan with a status other than denied for the logged in partner.
