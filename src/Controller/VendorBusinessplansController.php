...
/**
 * VendorBusinessplans Controller
 *
 * @property App\Model\Table\VendorBusinessplansTable $VendorBusinessplans
 */
class VendorBusinessplansController extends AppController {
    ...
    public function index() {
        $this->loadModel('VendorViewTracks');
            ...
            'contain' => ['Partners', 'Financialquarters', 'Vendors','BusinesplanCampaigns','BusinesplanCampaigns.Campaigns'],
                ...
                $bps_partners = $this->Businesplans->find('list')->select(['partner_id'])->group(['partner_id'])->toArray();
                $bps_quarters = $this->Businesplans->find('list')->select(['financialquarter_id'])->group(['financialquarter_id'])->toArray();
                $bps_campaigns = $this->BusinesplanCampaigns->find('list')->select(['campaign_id'])->group(['campaign_id'])->toArray();
                ...
                $myviewedbpln   =   $this->VendorViewTracks->find('all')
                                                            ->where(['vendor_id'=>$this->Auth->user('vendor_id'),'type'=>'Businessplan'])
                                                            ->hydrate(false);
 ...
 */
    ...
    global $businesplan;
        ...
        /*
         * Function to filter businessplans list using ajax
         */
        ...
        function ajaxdynamicfilter() {
             ...
             $businessplan = $this->Businesplans
                ->find('all')
            ...
            if(count($whereconditions)>0)
                $businessplan->where($whereconditions);

            $businessplan->toArray();

            $partners = array();
            $quarters = array();
            $statuses = array();
            $businessplans = array();
            foreach($businessplan as $bp) {
                ...
                $businessplans[$bp->id] = $bp->id;
            }

            $campaign = $this->BusinesplanCampaigns
                ->find('all')
                ->contain(['Campaigns'])
                ->where(['BusinesplanCampaigns.businesplan_id IN'=>$businessplans])
                ->toArray();
        ...
        /*
         * Function to insert campaigns to partner campaigns table on business plan approval....
         */
