// This automatically adds the correct classes to radio buttons and their labels so the CSS can do it's work
$(document).ready(function(){
  $('input[type="radio"]').addClass('css-checkbox');
  $('input[type="radio"]+label').addClass('css-label');
});



/* JH: This script is supposed to make the CSS-only styling of the radio buttons work with IE8, but it seems to foul up in newer browsers, making it possible to select more than one button at a time.  Need to check the code when have more time JH
	 REFER: http://www.csscheckbox.com/radio-buttons
