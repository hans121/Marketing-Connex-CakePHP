		<!--	<div class="connex_override">
				<?= $this->Html->link(__('Save & Exit'), $last_visited_page,['class' => 'pull-left btn btn-cancel exitsave', 'rel' => $last_visited_page]); ?>
				<a id="preview" href="#" class="pull-left btn btn-cancel ">Preview</a>			
				<div class="btn-group" id="templateselector">
				  <button type="button" class="btn btn-primary" rel="1">Template 1</button>
				  <button type="button" class="btn btn-primary" rel="2">Template 2</button>
				  <button type="button" class="btn btn-primary" rel="3">Template 3</button>
				</div>	
			</div>-->
		    ...
		    var beeConfig = {
		     ...
		     //theme: 'classic', //'dark|light|classic',
			  ...
			  //  alert (htmlFile);
				request(
					'POST', 
					'<?php echo $this->Url->build([ "controller" => "EmailTemplates","action" => "ajaxSaveEmail",$id],true);?>',
			   		  {
			            email: htmlFile,
			            template: jsonFile
			          },
		      ...
		      function(token) {
		        BeePlugin.create(token, beeConfig, function(beePluginInstance) {
		          ...
		          request(
					...
					'<?php echo $this->Url->build([ "controller" => "EmailTemplates","action" => "ajaxLoadEmail",$id],true);?>',
			   		  {
			            data: 1
			          },
		            'multipart/form-data',
