 ...
 */
class HelpMenusController extends AppController {
 ...
 */
    ...
    public function add($parent_id) {
        if ($this->request->is('post')) {
            ...
            $menu = $this->HelpMenus->newEntity($this->request->data);
            if ($this->HelpMenus->save($menu))
            ...
            return $this->redirect(['controller' => 'HelpPages','action' => 'navigate',$this->request->data['parent_id']]);
        }
        else
            $menu = $this->HelpMenus->newEntity([]);
 ...
 */
    ...
    public function edit($parent_id = 0, $menu_id) {
        $menu = $this->HelpMenus->get($menu_id);
        if ($this->request->is(['post','patch','put'])) {
            $this->request->data['modified_by'] = $this->user['id'];
            $menu = $this->HelpMenus->patchEntity($menu, $this->request->data);
            if ($this->HelpMenus->save($menu))
                $this->Flash->success('The menu has been saved.');
            else
                $this->Flash->error('Sorry, the menu could not be saved. Please try again. If you continue to experience problems, please contact Customer Support.');
 ...
 */
	public function delete($parent_id = 0,$menu_id) {
        if (isset($menu_id)) {
            $menu = $this->HelpMenus->get($menu_id, ['contain'=>['HelpPages']]);
            $submenus = $this->HelpMenus->find()->where(['parent_id'=>$menu_id]);
            if(count($menu->vendor_pages)<=0 && $submenus->count()<=0) {
                $entity = $this->HelpMenus->get($menu_id);
                if($this->HelpMenus->delete($entity))
                {
        ...
        return $this->redirect(['controller'=>'HelpPages','action'=>'navigate',$parent_id]);
	}
 ...
 */
    public function rename() {
        if ($this->request->is('post')) {
            $this->request->data['modified_by'] = $this->user['id'];
            $menu = $this->HelpMenus->get($this->request->data['id'], ['contain'=>['HelpPages']]);            
            $menu = $this->HelpMenus->patchEntity($menu,$this->request->data);
            if($this->HelpMenus->save( $menu ))
            {
                die('SUCCESS');
