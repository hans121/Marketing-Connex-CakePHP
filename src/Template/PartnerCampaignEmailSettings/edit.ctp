<script>
	...
	function sendtestmail(){
	  ...
	  $.ajax({
	    url: "<?php echo $this->Url->build([ "controller" => "PartnerCampaignEmailSettings","action" => "ajaxsenttestemail"],true);?>",
	    type: "POST",
	    data: formData,
	    async: false,
	    success: function (msg) {
...
<script>
	$(document).ready(function() {
	  ...
	  showsubjectoptions(<?=$partnerCampaignEmailSetting->campaign_id?>);
	...
	<div class="row table-title partner-table-title">
		...
		<div class="partner-sub-menu clearfix">
			...
			<div class="col-xs-5">
				...
				<div class="breadcrumbs">
					<?php
						...
						$this->Html->addCrumb('edit', ['controller' => 'PartnerCampaignEmailSettings', 'action' => 'edit', $partnerCampaignEmailSetting->id]);
			...
			<div class="col-xs-7">
		      ...
		      <?= $this->Html->link(__('Add new'), ['controller' => 'PartnerCampaignEmailSettings', 'action' => 'add'],['class'=>'btn btn-lg pull-right']); ?>
				</div>
			</div>
  ...
  <?php echo $this->Flash->render(); echo $this->Flash->render('auth'); ?>
	...
	<div class="partnerCampaignEmailSettings form col-centered col-lg-10 col-md-10 col-sm-10">
	  ...
	  ?>
		...
		<?php
			echo $this->Form->input('campaign_id', ['option'=>$campaigns,'data-live-search'=>true]);
	    echo $this->Form->input('email_template_id', ['type'=>'hidden']);
	    echo $this->Form->input('partner_campaign_id', ['type'=>'hidden']);
	    echo $this->Form->input('from_name',['label'=>"'From' name"]);
	    echo $this->Form->input('from_email',['type'=>'email','label'=>"'From' email address"]);
		...
		<div class="row checkbox_group">
		    ...
		    <div class="col-md-2 col-xs-6">
				<div class="onoffswitch">
					<?= $this->Form->checkbox('sntlater' ,['checked'=>'checked', 'class'=>'onoffswitch-checkbox', 'id'=>'sntlater', 'name'=>'sntlater']) ?>
	  ...
	  <div class="row checkbox_group">
		    ...
		    <?php if($twitter_isauth) { ?>
				<div class="onoffswitch">
					<?= $this->Form->checkbox('post_tweet' ,['class'=>'onoffswitch-checkbox', 'id'=>'auto-tweet','checked'=>$partnerCampaignEmailSetting->post_tweet=='1'?true:false,'disabled'=>$twitter_isauth?false:true]) ?>
	  ...
	  <div id="facebookbox" class="row">
		...
		<?php
		$facebook_pages_selected = array();
		$facebook_pages_stored = explode(',',$partnerCampaignEmailSetting->facebook_pages);
		foreach($facebook_pages_stored as $fb_page)
		{
			$page = explode(':',$fb_page);
			$facebook_pages_selected[] = $page[0];
		}
		
		foreach($facebook_pages as $pages){
			foreach($pages as $page) {
				if($page->id) {
					echo $this->Form->input('facebook_pages[]', ['type'=>'checkbox','value'=>$page->id.':'.$page->access_token,'label'=>$page->name,'checked'=>in_array($page->id,$facebook_pages_selected),'hiddenField' => false]);
				}
			}
		}
		?>
	  ...
	  <div class="row checkbox_group">
		    ...
		    <?php if($linkedin_isauth): ?>
				<div class="onoffswitch">
					<?= $this->Form->checkbox('post_linkedin' ,['class'=>'onoffswitch-checkbox', 'id'=>'post-linkedin','checked'=>$partnerCampaignEmailSetting->post_linkedin=='1'?true:false,'disabled'=>$linkedin_isauth?false:true]) ?>
	  ...
	  <div id="linkedinbox" class="row">
		...
		<div class="clearfix">&nbsp;</div>
	  	<label class="col-md-4 col-xs-6 control-label" for="linkedin_personal">
		...
		<div class="col-md-8 col-xs-6">
	  	...
	  	<label class="col-md-4 col-xs-6 control-label" for="linkedin_companies">
				<?= __('Post to your Linkedin company pages ('.$linkedin_companies->_total.')')?>
		</label>
		<div class="col-md-8 col-xs-6">
		<?php
			$linkedin_companies_selected = explode(',',$partnerCampaignEmailSetting->linkedin_companies);
			foreach($linkedin_companies->values as $company){
				echo $this->Form->input('linkedin_companies[]', ['type'=>'checkbox','value'=>$company->id,'label'=>$company->name,'checked'=>in_array($company->id,$linkedin_companies_selected),'hiddenField' => false]);
			}
		?>
		</div>
		<?php endif; ?>
		...
		?>
	      ...
	      ?>
		      ...
		      <span id="sentschedulebtn"><?= $this->Html->link(__('Schedule'), '#',['class'=> 'pull-right btn btn-lg btn-pad-left ','onclick'=>'schedule();']); ?></span>
					<?= $this->Html->link(__('Save as draft').' <i class="fa fa-pencil"></i>', '#',['escape' => false, 'class' => 'btn btn-lg btn-cancel btn-pad-left pull-right','onclick'=>'savedraft();']); ?>
		      <?= $this->Html->link(__('Send test').' <i class="fa fa-send"></i>', '#',['escape' => false, 'class' => 'btn btn-lg btn-cancel btn-pad-left pull-right','onclick'=>'sendtestmail();']); ?>
